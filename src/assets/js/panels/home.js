...
class Home {
    ...
    async initLaunch() {
        document.querySelector('.play-btn').addEventListener('click', async () => {
            ...
            let opts = {
                url: 'https://raw.githubusercontent.com/PerrierBouteille/Khebekoah-Launcher/main/url.json',
                authenticator: account,
                timeout: 10000,
                path: `${dataDirectory}/.Khebekoah`,
                version: "1.8.9",
    ...
    async initStatusServer() {
        fetch('https://mcapi.us/server/status?ip=mc.hypixel.net&port=25565')
        .then(response => response.json())
        .then(data => {
            let playersConnecteds = data.players.now
            playersConnected.textContent = playersConnecteds
        });

        let nameServer = document.querySelector('.name');
        ...
        let serverPing = new Status(this.config.status.ip, this.config.status.port).getStatus();
